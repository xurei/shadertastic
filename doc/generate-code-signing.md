...
TEMPLATE:
```bash
openssl req -x509 -newkey rsa:2048 \                                                                                                                ±[●●][main]
  -keyout private.key \
  -out certificate.crt \
  -days 365 \
  -nodes \
  -subj "/CN=YourCompanyName"
```
ACTUAL COMMAND:
```bash
openssl req -x509 -newkey rsa:2048 \                                                                                                                ±[●●][main]
...
##### Linux
```bash
openssl pkcs12 -export \                                                                                                                            ±[●●][main]
  -out certificate.pfx \
  -inkey private.key \
  -in certificate.crt
```

#### Step 4 : Encode the pfx with certutil or base64
##### Windows
```powershell
certutil -encode certificate.pfx certificate.pfx.b64
```
##### Linux
```bash
{
  echo "-----BEGIN CERTIFICATE-----"
  base64 -w 64 certificate.pfx
...
Add a repository secret named `WINDOWS_CERTIFICATE` with the content of `certificate.pfx.b64`.  
...
Also, you should probably delete the certificate.pfx.b64 and certificate.pfx files, and store them in a more secure place.
