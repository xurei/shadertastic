...
namespace effect_parameter_bool_detail {
    using base_param = effect_param_with_unique_prop<effect_parameter>;
}
class effect_parameter_bool : public effect_parameter_bool_detail::base_param {
    private:
        bool default_value{};

    public:
        // Note: the shaders need a 4-byte data for booleans, this is NOT a mistake to use sizeof(int).
        explicit effect_parameter_bool(gs_eparam_t *shader_param) : effect_parameter_bool_detail::base_param(sizeof(int), shader_param) {
        }
